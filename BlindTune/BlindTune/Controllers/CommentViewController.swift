...
class CommentViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextViewDelegate{
    ...
    override func viewDidLoad() {
        ...
        if commentArray.isEmpty {
            ...
            let alertVC = UIAlertController(title: "", message: "There are no comments on this post", preferredStyle: .alert)
...
//        timer.invalidate()
    ...
    @IBAction func audioButtonClicked(_ sender: Any) {
        ...
        controller.audioPostDic = self.audioPostDic
    ...
    ReportCommentViewController
        ...
        controller.prevController = "comment"
        self.present(controller, animated: true, completion: nil)
    }
    ...
    @objc func replyButtonClicked(sender:UIButton)
    {
        ...
        var postDict = commentArray[sender.tag]
        postDict["userID"] = self.audioPostDic["userID"]
        ...
        let tempStringArray = (postDict["audioName"] as! String).components(separatedBy: ".")
        ...
        controller.audioPostDic = postDict
        controller.isFromCommentController = true
        controller.userIdSet = tempSet
...
//        }
    ...
    @objc func usrerCountButtonClicked(sender:UIButton)
    {
        ...
        let tempDic = self.commentArray[sender.tag]
        
        let tempStringArray = (tempDic["audioName"] as! String).components(separatedBy: ".")
        
        var tempArray = self.allCommentArray.filter { (tempDic) -> Bool in
            tempDic["postId"] as! String == tempStringArray[0]
        }
...
//        return tempSet.count
        ...
        let controller = self.storyboard?.instantiateViewController(withIdentifier: "UserCommentViewController") as! UserCommentViewController
        controller.commentArray = tempArray
        controller.audioPostDic = tempDic
        controller.userIdSet = tempSet
        self.navigationController?.pushViewController(controller, animated: true)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let audioTableCell = tableView.dequeueReusableCell(withIdentifier: "audioCommentCell") as! AudioCommentTableViewCell
        let textTableCell = tableView.dequeueReusableCell(withIdentifier: "textCommentCell") as! TextCommentTableViewCell
...
extension CommentViewController: GrowingTextViewDelegate{
            ...
            if !(AppDelegate.currentAudioPlayer.first!.isPlaying) {
                ...
                firebaseRefReply.observe(.value) { (snapshot) in
                    ...
                    if self.allCommentArray.count > self.commentArray.count {
                        ...
                        self.commentArray = self.commentArray.filter { (tempDic) -> Bool in
                            let array = (self.audioPostDic["postId"] as! String).components(separatedBy: ".")
